...
type ReadableAxiosError = _ReadableAxiosError &
  Required<Pick<_ReadableAxiosError, 'response'>>;
...
export function setPopupError(msg: string) {
  const { dispatch, popupActions } = store;
  dispatch(popupActions.setError(msg));
}
